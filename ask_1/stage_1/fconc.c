 ...
 * Last Modified : Thu Nov 24 12:31:54 2011
...
int main(int argc, char ** argv)
{
  ...
  {
    TMP = open("fconc.out.tmp",W_FLAGS,C_PERMS);
    if (TMP < 0)
    {
    ...
    write_file(OUT,"fconc.out.tmp");
    lock.l_type = F_UNLCK;
    fcntl(OUT,F_SETLK,&lock);
    close (OUT);
    if (unlink("fconc.out.tmp") != 0)
    {
      perror("Error deleting temporary file, please remove fconc.out.tmp\n");
      exit(EX__BASE);
    }
  }
